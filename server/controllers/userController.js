...
const registerUser = asyncHandler(async (req, res) => {
  ...
  if (userExists) {
    return res
      .status(400)
      .json({ status: false, message: "Correo electrónico ya existe" });
  }
...
const logoutUser = (req, res) => {
  ...
  res.status(200).json({ message: "Cerrada sesión exitosamente" });
};
...
const changeUserPassword = asyncHandler(async (req, res) => {
  ...
  if (userId === "65ff94c7bb2de638d0c73f63") {
    return res.status(404).json({
      status: false,
      message: "Este es un usuario de prueba. No puedes cambiar la contraseña. Gracias!!!",
    });
  }
...
const deleteUserProfile = asyncHandler(async (req, res) => {
  ...
  res.status(200).json({ status: true, message: "Usuario eliminado exitosamente." });
});
